...
export async function handleTaskDelete(props: HandleTaskDeleteProps) {
  ...
  const response = await handleAxiosCall<undefined, GenericMessageResponse>({
    method: "delete",
    url: `${process.env.REACT_APP_API_URL}/api/v1/tasks/${elementId}`,
    needAuth: true,
  });
...
export async function handleTaskUpdate(
  ...
  const response = await handleAxiosCall<TaskRequest, TaskResponse<TaskObject>>(
    {
      method: "patch",
      url: `${process.env.REACT_APP_API_URL}/api/v1/tasks/${element.task.id}`,
      data: data,
      needAuth: true,
    }
  );
...
export async function handleTaskFilter(props: HandleTaskFilterProps) {
  ...
  >({
    method: "post",
    url: `${process.env.REACT_APP_API_URL}/api/v1/groups/${groupState.group?.id}/filter_tasks`,
    data: state,
    needAuth: true,
  });
